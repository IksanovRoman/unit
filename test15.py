...
def tab1():
    ...
    def add_customer_table():
        ...
        Button(app2, text="Добавить",command=add_information_customer).grid(row=0, column=2, padx=5)
    ...
    def delete_customer_table():
        ...
        Button(app2, text="Удалить",command=delete_information_customer).grid(column=2,row=0, padx=5)
    ...
    def update_customer_table():
        ...
        def update_customer_check_id():
            ...
            if text1.get()=="":
                ...
                Button(app2, text="Изменить имена заказчиков", command=update_customer).grid(column=2,row=2)
...
def tab2():
    ...
    def add_tools_table():
        ...
        #кнопка для добавления данных в таблицу        
        Button(app2, text="Добавить",command=add_information_tools).grid(row=0, column=2, padx=5)
    ...
    def update_tools_table():
        ...
        def update_tools_check_id():
            ...
            if text1.get()=="":
                ...
                #данная кнопка добавит данные в табличку
                Button(app2, text="Изменить средство защиты", command=update_tool).grid(column=1,row=3, pady=5)

        #кнопка подтверждения
        Button(app2, text="Подтвердить", command=update_tools_check_id).grid(column=2,row=0, padx=5)
...
def tab3():
    ...
    def show_licenses_table():
        cursor.execute(f"select l.id, c.name_short, t.license_name, l.licenses_date_start, l.licenses_date_end, l.key_techsupport_name, l.key_date_end from licenses as l join customer as c on c.id = l.customerid join tools as t on t.id = l.toolid order by l.id asc")
        mytable = from_db_cursor(cursor)
