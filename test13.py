    ...
    def add_customer_table():
        ...
        def add_information_customer():
            ...
            if t2=="" or t3=="": #если одно из поле пустой, то дальше не пойдем
                ...
                cursor.execute("select name_short, name_full from customer order by id asc") #проверим нет ли такого заказчика
                for row in cursor:
    ...
    def delete_customer_table():
        ...
        def delete_information_customer():
            ...
            if text1.get()!="":
                ...
                answer = messagebox.askyesno(title="Вопрос", message="Удалить данные из таблицы? Внимание, все дочерние записи из других таблиц тоже удалятся.") #вопрос подтверждения
                ...
                def update_customer():
                    ...
                    if t2!="" and t3!="" :
                        ...
                        if k:
                            ...
                            customerid=0 #найдем id которое поменяем
                            cursor.execute(f"select id from customer where name_short='{text1.get()}'")
                            for row in cursor:
                                customerid = row[0] #здесь хранится id заказчика
    ...
    def add_tools_table():
        ...
        def add_information_tools():
            ...
            if t2=="": #если одно из полей пустое, то дальше не пойдем
                ...
                cursor.execute("select license_name from tools order by id asc") #проверим нет ли такого заказчика
                for row in cursor:
                    if t2==row[0]:
                        k=1
                if k:
